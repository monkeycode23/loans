...
const ClientCard = ({ name, email, profileImage,id }) => {
...
const GuidedModal = ({id}) => {
    ...
    const addLoan =async (data) => {
      ...
      console.log()
    await window.sqlite.query(`INSERT INTO loans (amount,interes_percentage,installments,aproved_date,lender_id,client_id) 
        VALUES
        (${values})
        `) 
    ...
    <div className="max-w-sm mx-auto bg-white shadow-lg rounded-lg p-6 border border-gray-200">
      ...
      <div className="flex items-center">
        ...
        <div className="ml-4">
          ...
          <p className="text-sm text-gray-500">Total préstamos: {loans}</p>
        </div>
      </div>
      ...
      {/* Action Button */}
      <button className="mt-4 w-full bg-red-500 hover:bg-red-600 text-white py-2 px-4 rounded-lg font-medium">
        Anular
      </button>
    </div>
  );
